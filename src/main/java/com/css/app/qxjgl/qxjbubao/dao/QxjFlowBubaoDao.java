...
public interface QxjFlowBubaoDao extends BaseDao<QxjFlowBubao> {
    ...
    @Select("select top 1 * from (select * from QXJ_FLOW_BUBAO where id=#{0} and COMPLETE_FLAG='0' order By CREATED_TIME ASC)")
    QxjFlowBubao queryLastBuBaoUser(String id);

    List<QxjFlowBubao> queryAllBuBaoUser(String id);

    @Delete("delete from QXJ_FLOW_BUBAO where id=#{0} and COMPLETE_FLAG='0' and RECEIVE_ID = #{1}")
