...
import com.css.addbase.RestTemplateUtil;
...
public class BaseAppOrgMappedServiceImpl implements BaseAppOrgMappedService {
	...
	private static HashMap<String, List<BaseAppOrgMapped>>  baseAppOrgMappedMap= new HashMap<String, List<BaseAppOrgMapped>>();
...
//		String bareauId = getBareauByUserId(userId);
	...
	public List<BaseAppOrgMapped> getMappedData(String appLevel, String orgId, AppType appType) {
		...
		return getMappedData(appLevel,orgId,appType.getType());
	}
	
	@Override
	public List<BaseAppOrgMapped> getMappedData(String appLevel, String orgId, String type) {
		List<BaseAppOrgMapped> baseAppOrgMappeds = null;
		//缓存配置信息cacheKey
		String cacheKey=appLevel+":"+type+":"+orgId;
		if (baseAppOrgMappedMap.containsKey(cacheKey)) {
			//从缓存中读取配置信息
			baseAppOrgMappeds = baseAppOrgMappedMap.get(cacheKey);
			StringBuilder cacheValue = new StringBuilder();
			for (BaseAppOrgMapped baseAppOrgMapped:baseAppOrgMappeds) {
				cacheValue.append("{appId:");
				cacheValue.append(baseAppOrgMapped.getAppId());
				cacheValue.append(",appSecret:");
				cacheValue.append(baseAppOrgMapped.getAppSecret());
				cacheValue.append(",url:");
				cacheValue.append(baseAppOrgMapped.getUrl());
				cacheValue.append("}");
			}
			System.out.println("缓存配置信息key："+cacheKey+"缓存信息："+cacheValue);
			return baseAppOrgMappeds;
		}
		...
		if (StringUtils.equals(gatewayConfig, "true")) {
			...
			baseAppOrgMappeds = JSONArray.parseArray(jsons, BaseAppOrgMapped.class);
			//更新缓存信息
			baseAppOrgMappedMap.put(cacheKey, baseAppOrgMappeds);
			return baseAppOrgMappeds;
		}
		// 查询本地数据库
		Map<String, Object> map = new HashMap<String, Object>();
		...
		baseAppOrgMappeds = baseAppOrgMappedDao.queryList(map);
		//更新缓存信息
		baseAppOrgMappedMap.put(cacheKey, baseAppOrgMappeds);
		return baseAppOrgMappeds;
	}
	...
	public void clearConfigCache() {
	    System.out.println("清除缓存数量:"+baseAppOrgMappedMap.size());
		baseAppOrgMappedMap.clear();
	}
	
	public BaseAppOrgMapped queryObject(String appId) {
		return baseAppOrgMappedDao.queryObject(appId);
	}
