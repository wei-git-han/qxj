...
public interface BaseAppOrganDao extends BaseDao<BaseAppOrgan> {
	 ...
	 * @author gengds
	 */
	@Select("select * from BASE_APP_ORGAN start with ID= #{id} connect by prior ID = PARENT_ID ORDER BY ID")
	List<BaseAppOrgan> findAllByparentId(String id);
	
	/**
	 * 根据ID获取该部门及其全部子部门信息(有序
	 */
	@Select("select * from BASE_APP_ORGAN start with ID= #{id} connect by prior ID = PARENT_ID ORDER BY SORT ASC")
