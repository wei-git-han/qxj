...
function initselectNew(id, arry) {
    $("#" + id).html("");
    var html = "";
    $.each(arry, function(i) {
        html += '<option value=' + arry[i].value + '>' + arry[i].text + '</option>';
...
//修改页面参数并刷新
function setParams(obj){ //{key:'val'}
	if(!obj){
		obj = {}
	}
	obj.isReload = true;
...
function getStrParams(str,name){
	var reg=new RegExp("(^|&)"+name+"=([^&]*)(&|$)");
...
//		};
		...
		obj.onResizeColumn = function(a1,a2){
...
var initcheckgroup = function(pobj){
	...
	data1.templateId = templateId;//表格配置模板id
	$ajax({
		url:pobj.url1,
		data:data1,
		success:function(data){
			...
			getdatafn = function(){
				...
				$(".columns input").each(function(i){
					...
					if($(".auto input:checked").val()==0){
						...
						obj.width = parseInt($(".datagrid [field="+field+"]").width(),10)-7;
					}else{
						obj.width = parseInt($(".datagrid [field="+field+"]").width(),10)-7;
					}
					obj.number = i;
			...
			$(".auto input").click(function(){
				...
				$(".columns input").each(function(){
					...
					item.hidden = !this.checked;
					//item.width = parseInt($(this).attr("data1"),10);
					item.width = parseInt($(".datagrid [field="+field+"]").width(),10)-7;
					//if(value==0){
					//	item.width = parseInt($(this).attr("data1"),10);
					//}else{
					//	item.width = parseInt($(".datagrid [field="+field+"]").width(),10);
...
var getCookie = function(cookieName){
	...
	if(document.cookie&&document.cookie!=''){
		...
		for(var i=0;i<cookie.length;i++){
			...
			if(name[0].trim()==cookieName.trim()){
				cookieValue = name[1]
			}
		}
	};
...
var getFormatDate = function(){
	...
	var formatDate = date.getFullYear() +""+ month +""+ day +""+ hours +""+ minutes +""+ seconds;
	return formatDate;
}
//上传图片（支持同时上传多个图片）
//上传图片（支持同时上传多个图片）
jQuery.fn.extend({ //成员函数
	createfile:function(obj){
			...
			$.each(filelist,function(i,o){
				if(view=="view1"){
					$(".filelist[name="+targetid+"]").append(
						'<div style="display:block;position:relative;text-align: left;margin-right:20px;margin-bottom: 10px;" id="filecont_'+targetid+'_'+o.fileServerId+'" class="fileview_'+targetid+' hqds" data="'+o.fileServerId+'">'+
						'	<img src="/app/gwcl/common/images/fileimg.png" /><a onclick="downloadfn(\''+o.fileServerId+'\')" title="下载" style="margin-left:3px;margin-right:8px;">'+o.fileName+'</a>'+
					...
					$(".filelist[name="+targetid+"]").append(
						'<div style="display:inline-block;'+cssStyle+';position:relative;float:left;margin-right:20px;margin-bottom:10px;" id="filecont_'+targetid+'_'+o.code+'" class="fileview_'+targetid+' hqds" data="'+o.code+'">'+
		...
		$("#"+targetid).click(function(){
			...
			if(view=="view1"){
				$(".filelist[name="+targetid+"]").append(
					'<div style="display:block;position:relative;display:none;text-align: left;margin-bottom:10px;" id="filecont_'+targetid+'_'+filen+'" class="hqds">'+
					'	<input type="file" accept=".pdf,.ofd,.doc,.docx,.wps" name="'+name+'" style="display:none;" id="file_'+targetid+'_'+filen+'"/>'+
					'</div>'
				);
			}else if(view=="view2"){
				$(".filelist[name="+targetid+"]").append(
					'<div style="display:inline-block;'+cssStyle+';position:relative;display:none;float:left;margin-bottom:10px;" id="filecont_'+targetid+'_'+filen+'" class="hqds">'+
			...
			$("#file_"+targetid+"_"+filen).change(function(){
				...
				if(view=="view1"){
					...
					$("#filecont_"+targetid+"_"+filen).append(
						'	<img src="/app/gwcl/common/images/fileimg.png" /><font class="filename">'+filename+'</font>'+
...
jQuery.fn.extend({
	createfiles:function(obj){
		...
		$(".filelist[name="+targetid+"]").html("");
		$(".filelist[name="+targetid+"]").append(
			'<div style="display:none;" class="fileinputcont"></div>'
		);
		...
		if(obj.initdata){
			...
			if(view=="view1"){
				$.each(filelist,function(i,o){
					$(".filelist[name="+targetid+"]").append(
						'<div style="display:block;position:relative;margin-left:10px;" id="'+o.id+'" class="fileview_'+targetid+'" >'+
						'	<font class="over-text">'+o.documentTitle+'</font>'+
						'	<a href="'+o.load+'" class="fileload2">下载</a>'+
						'	<a class="close_'+targetid+' filedelete2" >删除</a>'+
						'</div>'
					);
				});
			}else if(view=="view2"){
				$.each(filelist,function(i,o){
					$(".filelist[name="+targetid+"]").append(
						'<div style="display:inline-block;width:100px;height:100px;position:relative;margin-left:10px;" id="'+o.id+'" class="fileview_'+targetid+'" >'+
						'	<div style="width:20px;height:20px;position:absolute;top:0;right:0;text-align:center;background:#cccccc;cursor:pointer;color:#fff;"  class="close_'+targetid+'" >x</div>'+
						'	<div style="width:100%;height:100%;">'+
						'		<img src="'+o.load+'"  style="width:100%;height:100%;"/>'+
						'	</div>'+
						'</div>'
			...
			$(".close_"+targetid).unbind("click");
			$(".close_"+targetid).click(function(){
				$(this).parent().remove();
				deletefilekey.push($(this).parent().attr("id"));
			});
		}
		$('.filemessage[name='+targetid+']').hide();
		$("#"+targetid).unbind("click");
		$("#"+targetid).click(function(){
			...
			$(".filelist[name="+targetid+"] .fileinputcont").append(
				'	<input type="file" name="'+name+'" style="display:none;" id="file_'+targetid+'_'+filen+'" '+multiple+' accept="'+accept+'" />'
			);
			
			$("#file_"+targetid+"_"+filen).unbind("change");
			$("#file_"+targetid+"_"+filen).change(function(){
				...
				$.each(filearray,function(){
					...
					var filename = (array.slice(0,array.length-1)).join(".");
					var filetype = array[array.length-1];
					
					if(!type){
					}else{
						var type2 = type.join(",");
						if(type2.indexOf(filetype)==-1){
							error1.push(filetype);
						}
					}
					
					var otype="jpg,png";
					if(otype.indexOf(filetype)==-1){
						error3.push(filetype);
					}
				...
				if(error1.length>0){
					$('.filemessage[name='+targetid+']').html('不支持.'+error1.join(",")+'文件类型');
					$('.filemessage[name='+targetid+']').show();
					$(this).remove();
					return;
				};
				...
				if(error2.length>0){
					var maxsize = obj.maxsize;
					$('.filemessage[name='+targetid+']').html('您上传的文件'+error2.join(",")+'超过'+(maxsize/1024/1024).toFixed(2)+'M，请重新上传!');
					$('.filemessage[name='+targetid+']').show();
					$(this).remove();
					return;
				};
				...
				if(view=="view2"){
					if(error3.length>0){
						$('.filemessage[name='+targetid+']').html('当前视图不支持.'+error3.join(",")+'文件类型！');
						$('.filemessage[name='+targetid+']').show();
						$(this).remove();
						return;
					}
				...
				if(yn==false){
					if($(".fileview_"+targetid).length!=0){
						deletefilekey.push($(".fileview_"+targetid).attr("id"));
						$(".fileview_"+targetid).remove();
					};
					$(".filecont_"+targetid).remove();
				};
				...
				if(view=="view1"){
					...
					$.each(filearray,function(i){
						...
						$(".filelist[name="+targetid+"]").append(
							'<div style="display:block;position:relative;display:none;margin-left:10px;" class="filecont_'+targetid+'" id="filecont_'+targetid+'_'+filen+'_'+i+'" >'+
							'	<font class="over-text">'+name+'</font>'+
							'	<a class="close_'+targetid+' filedelete2" >删除</a>'+
							'</div>'
						);
					});
					...
					$.each(filearray,function(i){
						var name = this.name;
						$(".filelist[name="+targetid+"]").append(
							'<div style="display:inline-block;width:100px;height:100px;position:relative;display:none;margin-left:10px;" class="filecont_'+targetid+'" id="filecont_'+targetid+'_'+filen+'_'+i+'" >'+
							'	<div style="width:20px;height:20px;position:absolute;top:0;right:0;text-align:center;background:#cccccc;cursor:pointer;color:#fff;" class="close_'+targetid+'" >x</div>'+
							'	<div style="width:100%;height:100%;">'+
							'		<img id="img_'+targetid+'_'+filen+'_'+i+'" style="width:100%;height:100%;"/>'+
							'	</div>'+
							'</div>'
						);
						...
						var reader = new FileReader();
						reader.onload = function(re){
							$("#img_"+targetid+"_"+filen+"_"+i).attr("src",re.target.result);
						}
						reader.readAsDataURL(file);
					});
				...
				if(view=="view2"){
					$(".filecont_"+targetid).css("display","inline-block");
				}else{
					$(".filecont_"+targetid).css("display","block");
				}
...
// 新版人员树支持多选
jQuery.fn.extend({
    createNewUserTree: function(obj) {
        obj.target = $(this).attr("id");
        var gridobj = new createNewUserTree(obj);
        return gridobj;
    }
});
// 新版树支持多选
jQuery.fn.extend({
    createNewTree: function(obj) {
        obj.target = $(this).attr("id");
        var gridobj = new createNewTree(obj);
        return gridobj;
    }
});
var isclose = true;
function createNewUserTree(obj){
    var create = function(){
        $(".selecttree").css({
            width:"100%",
            height:"100%",
